...
public class CarScript : MonoBehaviour{ //Названия основного класса всегда должно совпадать с названием файла скрипта; MonoBehaviour это базовый класс, от которого наследуются все скрипты.
    ...
    void Start () { // метод Start() выполняется единожды, сразу после окончания загрузки сцены
		...
		currentFuel = fuelSize; //полный бак
	...
	void FixedUpdate() { 
		...
		frontWheel.motorSpeed = backWheel.motorSpeed; //уравниваем тягу передних и задних колёс
        ...
        if (grounded == true) 
        {
            if (ControlCar[0].clickedIs == true) //если газ нажат,то появляется зависимость от угла машины и.т.д. т.е. ускорение в итоге будет гладкое
            {
    ...
    void OnTriggerEnter2D(Collider2D trigger) //функция ,что будет происходить при сборе монеток машиной и финише
    {
		if (trigger.gameObject.tag == "coins") {
            ...
            coinSound.Play (); //включаем звук при сборе монет
            Destroy(trigger.gameObject); //GameObject монеток будет разрушаться
        }
